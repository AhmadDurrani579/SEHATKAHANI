...
class Speciality: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
...
//        self.navigationController?.navigationBar.shadowImage = UIImage()
    ...
    override func viewDidAppear(_ animated: Bool) {
        self.navigationController?.navigationBar.topItem?.title = "Speciality"
...
//        cell.layer.borderColor = #colorLiteral(red: 0.8391239047, green: 0.8392685056, blue: 0.8391147852, alpha: 1)
    ...
    func collectionView(_ collectionView: UICollectionView, shouldSelectItemAt indexPath: IndexPath) -> Bool {
        name = speci[indexPath.item].1
        return true
    }
    
    func specialites() {
        getData.getDrbySpeciality(url: "/api/MobileApp/findDoctor/find_by_speciality_all") { (data) in
            self.listOfDr = data
