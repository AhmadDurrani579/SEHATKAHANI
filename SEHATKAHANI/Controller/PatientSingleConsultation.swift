//
//  PatientSingleConsultation.swift
//  Sehat Kahani
//
...
struct About {
    var section: String!
    var rows: [(title: String, subtitle: String, des: String, img: String)]
}

import UIKit

class PatientSingleConsultation: UIViewController, UITableViewDataSource, UITableViewDelegate {
...
//        self.navigationController?.navigationBar.topItem?.title = "Consultation History"
        ...
        self.array.append(About(section: "Summery", rows: [(title: "Summery", subtitle: "", des: "", img: "")]))
...
//        headerImage.cornerRadius3()
        ...
        if let idString = ap.appointmentObj?.consultationId {
           getAllPendingAppointment(id: idString)
        }
        if let patientid = patientID {
            getAllPendingAppointment(id: patientid)
        }
    ...
    func getAllPendingAppointment(id: String) {
//        let doctorId = summaryOfDoctorObject?.consultantionId
...
//            cell.title.text = self.array[indexPath.section].section
        ...
        if self.array[indexPath.section].section == "Summery" {
            let cell  = tableView.dequeueReusableCell(withIdentifier: "SingleNoteCell", for: indexPath) as! SingleNoteCell
//            cell.title.text = self.array[indexPath.section].section
            cell.title.text  = "Summery"
        ...
        let cell2 = tableView.dequeueReusableCell(withIdentifier: "SingleNote2Cell", for: indexPath) as! SingleNote2Cell
            ...
            cell2.lblDose.text = self.consultent?.presCription![indexPath.row].drugName
